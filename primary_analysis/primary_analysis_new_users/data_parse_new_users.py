...
bounce rate, and aggregate differences for ONLY new users.
"""
...
def main(args):
    ...
    for i in range(0, len(uniqueDates)):
        ...
        for j in range(0, len(dataset)):
            if dataset['Date'][j] == uniqueDates[i]:
                if dataset['User Type'][j] == 'New User':
                    if dataset['Land'][j] == 1:
    ...
    primary_results.to_csv('secondary_results_new_users_only.csv')
    ...
    results.to_csv('secondary_summary_results_new_users_only.csv')
