...
bounce rate, and aggregate differences ONLY for users who landed on the viagogo homepage from a paid link on a search.
"""
...
def main(args):
    ...
    for i in range(0, len(uniqueDates)):
        ...
        for j in range(0, len(dataset)):
            if dataset['Date'][j] == uniqueDates[i]:
                if dataset['Channel'][j] == 'Paid Search':
                    if dataset['Land'][j] == 1:
    ...
    primary_results.to_csv('secondary_results_land_paid_search_only.csv')
    ...
    results.to_csv('secondary_summary_results_land_paid_search_only.csv')
