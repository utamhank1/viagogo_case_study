...
bounce rate, and aggregate differences ONLY for returning users who landed on the viagogo homepage from an email link.
"""
...
def main(args):
    ...
    for i in range(0, len(uniqueDates)):
        ...
        for j in range(0, len(dataset)):
            if dataset['Date'][j] == uniqueDates[i]:
                if dataset['Channel'][j] == 'Email' and dataset['User Type'][j] == 'Returning User':
                    if dataset['Land'][j] == 1:
    ...
    primary_results.to_csv('secondary_results_land_email_returning_users_only.csv')
    ...
    results.to_csv('secondary_summary_results_land_email_returning_users_only.csv')
